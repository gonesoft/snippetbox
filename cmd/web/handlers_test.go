...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		//{"Valid submission", "Bob", "gs@someone.com", "validPa$$word", csrfToken, http.StatusSeeOther, nil},
		...
		//{"short password", "Bob", "dupe@example.com", "pa$$word", csrfToken, http.StatusOK, []byte("This field is too short (minimum is 10 characters)")},
		//{"Duplicate email", "Bob", "dupe@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("Address is already in use")},
		{"Invalid CSRF token", "", "", "", "wrong token", http.StatusBadRequest, nil},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			form := url.Values{}
